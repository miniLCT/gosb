...
func ExampleBitSet() {
	bs, _ := NewFromHex("c020")
	fmt.Println("Origin:", bs.Binary(" "))
	not := bs.Not()
	fmt.Println("Not:", not.Binary(" "))
	fmt.Println("AndNot:", not.AndNot(New(1, 1)).Binary(" "))
	fmt.Println("And:", not.And(New(1<<1, 1<<1)).Binary(" "))
	fmt.Println("Or:", not.Or(New(1<<7, 1<<7)).Binary(" "))
	fmt.Println("Xor:", not.Xor(New(1<<7, 1<<7)).Binary(" "))
